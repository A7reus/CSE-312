...
add_action( 'init', 'uwa_pro_plugin_uwa_update_link', 10, 1 );

function uwa_pro_plugin_uwa_update_link() {
	global $uwa_update_link;
	$uwa_update_link = array(

		'pro_link' => 'https://bit.ly/3Yw7Oez',
...
if ( ! function_exists( 'uwa_free_plugin_action_links' ) ) {
	...
	function uwa_free_plugin_action_links( $links ) {
		...
		$links[] = '<a rel="nofollow" href="http://auctionplugin.net/" style="color: #389e38;font-weight: bold;" target="_blank">' . __( 'Get Pro', 'ultimate-woocommerce-auction' ) . '</a>';
	...
	if ( is_plugin_active( $pro_plugin ) ) {
		...
		function uwa_custom_message() {
			...
			<div class="notice notice-error is-dismissible">
				<p><?php esc_html_e( 'You are trying to activate Ultimate WooCommerce free version where as you already have its PRO version activated. Please deactivate Ultimate WooCommerce Pro version and then activate Free version.', 'ultimate-woocommerce-auction' ); ?></p>
			</div>
		...
		if ( ! class_exists( 'Ultimate_WooCommerce_Auction_Free' ) ) {
			...
			if ( ! defined( 'WOO_UA_VERSION' ) ) {
				define( 'WOO_UA_VERSION', '2.5.4' ); // plugin version
			}
			...
			class Ultimate_WooCommerce_Auction_Free {
				...
				public function uwa_pro_add_plugins_notice() {
					...
					$uwa_pro_add_plugins_notice_nonce = wp_create_nonce( 'uwa_pro_add_plugins_notice' );
					if ( ! isset( $uwa_pro_add_plugins_notice_nonce ) || ! wp_verify_nonce( $uwa_pro_add_plugins_notice_nonce, 'uwa_pro_add_plugins_notice' ) ) {
						wp_send_json_error( 'Nonce verification failed.' );
					}
				...
				public function uwa_free_install( $network_wide ) {
					...
					if ( $network_wide ) {
						...
						$site_ids = wp_cache_get( 'network_site_ids', 'plugin_network_sites' );
						if ( false === $site_ids ) {
							wp_cache_set( 'network_site_ids', $site_ids, 'plugin_network_sites', HOUR_IN_SECONDS );
						}
				...
				public function uwa_free_plugin_new_blog( $blog_id, $user_id, $domain, $path, $site_id, $meta ) {
					...
					$plugin_file = basename( __DIR__ ) . '/ultimate-woocommerce-auction.php';
				...
				public function ultimate_woocommerce_auction_place_bid( $url = false ) {
					...
					if ( ! isset( $_REQUEST['uwa_uwa_place_bid_nonce'] ) || ! wp_verify_nonce( $_REQUEST['uwa_uwa_place_bid_nonce'], 'uwa_uwa_place_bid' ) ) {
					...
					$bid = abs( round( (float) str_replace( ',', '.', $_REQUEST['uwa_bid_value'] ), wc_get_price_decimals() ) );
	...
	if ( ! function_exists( 'uwa_install_woocommerce_admin_notice' ) ) {
		...
		function uwa_install_woocommerce_admin_notice() {
			...
			<div class="updated" id="uwa-free-installer-notice" style="padding: 1em; position: relative;">
				<h2><?php esc_html_e( 'Your Ultimate WooCommerce Auction is almost ready!', 'ultimate-woocommerce-auction' ); ?></h2>

				<?php
				$plugin_file      = basename( __DIR__ ) . '/ultimate-woocommerce-auction.php';
				$core_plugin_file = 'woocommerce/woocommerce.php';
				?>
				<a href="<?php echo esc_url( wp_nonce_url( 'plugins.php?action=deactivate&plugin=' . $plugin_file . '&plugin_status=all&paged=1&s=', 'deactivate-plugin_' . $plugin_file ) ); ?>" class="notice-dismiss" style="text-decoration: none;" title="<?php esc_attr_e( 'Dismiss this notice', 'ultimate-woocommerce-auction' ); ?>"></a>

				<?php if ( file_exists( WP_PLUGIN_DIR . '/' . $core_plugin_file ) && is_plugin_inactive( 'woocommerce' ) ) : ?>
					<?php
					$plugin_name_Woo = 'WooCommerce';
						// Escape the string for safe output in HTML context.
						$plugin_name_escaped = esc_html( $plugin_name_Woo );
					?>
					<p><?php printf( wp_kses( 'You just need to activate the <strong>%s</strong> to make it functional.', 'ultimate-woocommerce-auction' ), esc_html( $plugin_name_escaped ) ); ?></p>

					<p><?php printf( wp_kses( 'You just need to activate the <strong>%s</strong> to make it functional.', 'ultimate-woocommerce-auction' ), 'WooCommerce' ); ?></p>
					<p>
						<a class="button button-primary" href="<?php echo esc_url( wp_nonce_url( 'plugins.php?action=activate&amp;plugin=' . $core_plugin_file . '&amp;plugin_status=all&amp;paged=1&amp;s&amp;_wpnonce=214569a558', 'activate-plugin_' . $core_plugin_file ) ); ?>" 
						title="<?php esc_html_e( 'Activate this plugin', 'ultimate-woocommerce-auction' ); ?>">
						<?php esc_html_e( 'Activate', 'ultimate-woocommerce-auction' ); ?></a>
					</p>
				<?php else : ?>
				
					<p><?php printf( wp_kses( 'You just need to install the %sCore Plugin%s to make it functional.', 'ultimate-woocommerce-auction' ), '<a target="_blank" href="https://wordpress.org/plugins/woocommerce/">', '</a>' ); ?></p>
